...
ProductLinks = []
for i in range(1,5):
    MinceurSite = requests.get("https://anais.tn/product-category/minceur/page/{}/".format(i)).text
    soup = BeautifulSoup(MinceurSite,'html.parser')
...
#Prod_Det
    try:
        ...
        Prod_Det = ("-")
    Minceur = {"Produit":Product_Name, "Prix initiale":New_Price, "Ancien prix":Old_Price, "Description":Prod_Det, "Lien":Links}    
    Anais_Minceur_Prod.append(Minceur)
    c += 1
    print("Completed ",c)

AnaisAllProduct_Minceur = pd.DataFrame(Anais_Minceur_Prod)

#To Excel
AnaisAllProduct_Minceur.to_excel("Minceur_AnaisProduct.xlsx",index=False)
