...
Cleopatre_Homme_Product = []
c = 0
ProductLinks = []

for i in range(1,9):
    Site = requests.get('https://www.cleopatre.tn/35-homme?page={}'.format(i)).text
...
#Old_Price
    try:
        Old_Price = soup2.find("div",{"class":"product-discount"}).text.replace("\n","")
    except:
        Old_Price = ("-")
#Discount
    try:
...
#Prod_Det
    try:
        Prod_Det=soup2.find("div",{"class":"std"}).text.replace("\n\n","")
    except:
        Prod_Det = ("-")
    ...
    Cleopatre_Homme_Product.append(Cleopatre)
    c += 1
    print("Completed",c)

HommeAllProduct = pd.DataFrame(Cleopatre_Homme_Product)

#To Execl
HommeAllProduct.to_excel("Produit_HommeCleopatre.xlsx",index=False)